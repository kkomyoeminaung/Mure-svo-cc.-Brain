...
print("📥 Pulling Gemma-2-2B locally... This may take a few minutes.")
# using subprocess to ensure it finishes pulling
...
if not os.path.exists('corpus.txt'):
    os.system("wget -qO corpus.txt https://raw.githubusercontent.com/karpathy/char-rnn/master/data/tinyshakespeare/input.txt")
...
def extract_with_gemma(chunk):
    ...
    try:
        # Increased timeout as generation might take a bit longer
        res = requests.post('http://localhost:11434/api/generate', 
                            json={"model": "gemma2:2b", "prompt": prompt, "stream": False, "format": "json"}, timeout=60)
        return json.loads(res.json()['response'])
    except Exception as e:
        return []

if len(rules) < TARGET_COUNT:
    print(f"🧠 Generating High-Quality Rules using Local Gemma...")
            ...
            if isinstance(result, list):
                for rule in result:
                    if isinstance(rule, dict) and 'cause' in rule and 'effect' in rule:
                        key = (str(rule['cause']).strip(), str(rule['effect']).strip())
                        if len(key[0]) > 2 and len(key[1]) > 2 and key not in seen_keys:
                            seen_keys.add(key)
                            rules.append(rule)
                            writer.write(rule)
                            pbar.update(1)
