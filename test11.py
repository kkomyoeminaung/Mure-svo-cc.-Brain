...
print("🚀 Initializing High-Speed Procedural Logic Engine (CPU Friendly)...")
...
def generate_math_rule():
    ...
    return {
        'cause': f"The mathematical expression is {a} {op[0]} {b}.",
        'effect': f"The calculated result evaluates to {op[1](a,b)}."
    }

def generate_logic_rule():
    subjects = ['Alice', 'Bob', 'Charlie', 'Delta System']
    states = ['active', 'offline', 'verified', 'compromised']
    actions = ['proceed', 'halt', 'escalate', 'lockdown']
    s = random.choice(subjects)
    st = random.choice(states)
    a = random.choice(actions)
    return {
        'cause': f"User status check: {s} is currently {st}.",
        'effect': f"The policy engine enforces the action: {a}."
    }

def generate_code_rule():
    vars = ['x', 'y', 'count', 'total']
    v = random.choice(vars)
    val = random.randint(1, 100)
    return {
        'cause': f"A loop increments {v} from 0 to {val} by 1.",
        'effect': f"The final value of {v} after the loop terminates is {val}."
    }
...
if len(rules) < TARGET_COUNT:
    print(f"⚡ Generating {TARGET_COUNT:,} High-Quality Procedural Logical Rules Instantly...")
    ...
    while len(rules) < TARGET_COUNT:
        batch = []
        for _ in range(min(batch_size, TARGET_COUNT - len(rules))):
    ...
    writer.close()
    print("\n✅ 5,000,000 Procedural Rules Generated Successfully!")
else:
